...
WORKFLOW_NAME = 'wmat_transfer_dummy_wsweep'
MAX_ITERATIONS = 3
...
FIXTURE_NOTICE = (
    'Every iteration reuses unmatched mounted DW and RR data. '
    'Weight updates validate workflow plumbing and plot metadata only.'
)
INITIAL_CONTEXT = {
    'wmat_multipliers': [
...
def get_workflow_params():
    """Return all serializable dummy sweep parameters."""
    return {
...
def get_run_id(workflow_params):
    """Generate the default dummy sweep result directory name."""
    stages = {
        stage['name']: stage
    ...
    return (
        f'exp_wmat_transfer_dummy_wsweep'
...
def get_stage_overrides(stage_name, iteration, iteration_context,
    ...
    """Attach current dummy weights and prior-stage corrections."""
    ...
    raise KeyError(f'Unknown dummy sweep stage: {stage_name}')
...
def finish_iteration(iteration, iteration_context, stage_results, history):
    """Record one dummy sweep step and return the next weight context."""
