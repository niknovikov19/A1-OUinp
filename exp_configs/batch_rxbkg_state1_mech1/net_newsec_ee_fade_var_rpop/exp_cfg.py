...
from batch_params import N_RATE_PTS, POP_FRZ_RATES
...
def gen_exp_name_sub(cfg):
    ...
    exp_name_sub += '_frz'
    for pop, rlim in POP_FRZ_RATES.items():
        exp_name_sub += f'_{pop}_{rlim[0]}_{rlim[1]}'
    exp_name_sub += f'_npts_{N_RATE_PTS}'
...
def post_run(sim):
    ...
    postfix = (f'{exp_id}_frz_{cfg.pop_frz}_pt_{cfg.rate_pt}')
