...
from batch_params import (
    N_SEEDS, OSC_F_VALUES, OSC_AMP_VALUES,
    PYR_POPS, PV_POPS, SOM_POPS, VIP_POPS, NGF_POPS,
    THAL_E_POPS, THAL_I_POPS, L2_POPS
)
...
def gen_exp_name_sub(cfg):
    ...
    exp_name_sub += (
        f'_nseed_{N_SEEDS}_nf_{len(OSC_F_VALUES)}_namp_{len(OSC_AMP_VALUES)}'
    )
...
def modify_network(sim):
    if sim.cfg.osc_f is None or sim.cfg.osc_amp is None:
        raise ValueError('sim.cfg.osc_f and sim.cfg.osc_amp must be set')

    tvec_np = np.arange(0.0, sim.cfg.duration, sim.cfg.dt)
    ivec_np = np.zeros_like(tvec_np, dtype=float)
    osc_mask = tvec_np >= OSC_T0
    ivec_np[osc_mask] = (
        float(sim.cfg.osc_amp) *
        np.sin(
            2 * np.pi * float(sim.cfg.osc_f) *
            (tvec_np[osc_mask] - OSC_T0) / 1000.0
        )
    )
    ...
    for cell in sim.net.cells:
        ...
        if pop_name not in POPS_USED:
            continue
        if 'soma' not in cell.secs:
            raise ValueError(
                f"Cell gid={cell.gid} pop={pop_name} has no 'soma' section"
            )

        clamp = h.IClamp(cell.secs['soma']['hObj'](0.5))
        clamp.delay = 0
        clamp.dur = sim.cfg.duration
        clamp.amp = 0

        tvec = h.Vector(tvec_np)
        ivec = h.Vector(ivec_np)
        ivec.play(clamp._ref_amp, tvec, True)

        sim.hr_osc_inputs.append({
            'cell_gid': cell.gid,
            'pop': pop_name,
            'hObj': clamp,
            'tvec': tvec,
...
def post_run(sim):
    ...
    postfix = (
        f'{exp_id}_seed_{cfg.seed_main}_f_{cfg.osc_f:g}_amp_{cfg.osc_amp:g}'
    )
