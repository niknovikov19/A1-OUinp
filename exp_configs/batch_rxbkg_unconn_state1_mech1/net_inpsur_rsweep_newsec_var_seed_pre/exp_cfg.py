...
from batch_params import N_SEEDS, POPS_PRE
...
def modify_net_params_2(cfg, params):
    ...
    # produce overlapping per-cell seed ranges [base+0 .. base+n_cells).
    # Scrambling makes collisions negligible (~n_cells/2^31 per pair of pops).
    ...
    # splits that into two uncorrelated sub-seeds (homogeneous process + thinning),
    # so every cell in every surrogate pop fires independently.
