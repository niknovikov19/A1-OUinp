...
def apply_exp_cfg(cfg):
    ...
    cfg.bkg_spike_inputs[POP_MAIN] = {
        'exc': {'r': 0, 'w': 0.1, 'sec': 'apic'},   # r will be set in batch
        'inh': {'r': 1e4, 'w': 0.1, 'sec': 'soma'},
    }
...
def post_run(sim):
    ...
    exp_name_sub += f'_rxi_{rxi}_wx_{wxe}_{wxi}'
    exp_name_sub += f'_wmult_{cfg.wmult}'

    # Generate filename postfix with param values
    exp_id = exp_name.split('_')[-1]
    postfix = (f'{exp_id}_rxe_{rxe}')
